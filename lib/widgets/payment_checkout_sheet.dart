...
class _PaymentCheckoutSheetState extends State<PaymentCheckoutSheet> {
  ...
  void _requireClientAuth() {
    if (ApiService.clientToken == null) {
      throw Exception('Necesitas iniciar sesi贸n como cliente antes de usar el checkout.');
    }
  }
  ...
  Future<void> _confirmPayment() async {
    ...
    if (intentId == null || intentId.isEmpty || clientSecret == null) {
      setState(() {
        _error = 'A煤n no hay intent de pago disponible.';
      });
      return;
    }
    _requireClientAuth();
  ...
  Future<void> _createIntent() async {
    if (widget.offerId <= 0) {
      throw Exception('Oferta inv醠ida para crear el pago.');
    }
    _requireClientAuth();
    final overrides = {
  ...
  Widget _buildStatusTile() {
    ...
    final description = status == 'SUCCEEDED'
        ? (assignedWorker != null
            ? 'El servicio qued贸 asignado a $assignedWorker.'
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                'Oferta #${widget.offerId} 路 ${serviceTitle?.isNotEmpty == true ? serviceTitle : 'Servicio'}',
